...
static int ip_output_device(struct ip_iface *iface, const uint8_t *data, size_t len, ip_addr_t dst)
{
    ...
    return net_device_output(NET_IFACE(iface)->dev, NET_IFACE(iface)->dev->type, data, len, hwaddr);
}
...
char *ip_addr_ntop(ip_addr_t src, char *addr, size_t len)
{
    uint8_t *u8;
    u8 = (uint8_t *)&src;
    snprintf(addr, len, "%d.%d.%d.%d", u8[0], u8[1], u8[2], u8[3]);
...
struct ip_iface *ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    errorf("ip_addr_pton() failure, addr=%s, addr=%d", unicast, iface->unicast);
